<div class="widget-content widget-content-area mb-3">
    <div class="row">
        <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12">
            <?php $image = isset($album->images[0]) ? $album->images[0]->url : base_url('assets/img/no-song.png'); ?>
            <div class="our-team-4">
                <div class="usr-profile">
                    <img src="<?= esc($image) ?>" alt="<?= $album->name ?>">
                </div>
                <div class="team-content">
                    <div class="team-detail">
                        <h3 class="usr-name">Album <?= $album->name ?></h3>
                    </div>
                    <div class="social-content">
                        <ul class="list-inline">
                            <li class="list-inline-item icon-twitter">
                                <a title="Play" class="play-album keterangan" href="javascript:void(0);" data-id="<?= esc($album->uri); ?>"><i class="flaticon-next2"></i></a>
                            </li>
                        </ul>
                    </div>
                </div>
    ...
    <div class="row">
        <div class="table-responsive">
            <table class="table table-hover table-sm table-custom">
                ...
                <tbody>
                    <?php foreach ($album->tracks->items as $key => $item) : ?>
                        <tr>
                            <td><?= $key + 1; ?></td>
                            <td><?= $item->name; ?></td>
                            <td><?= $item->name ?></td>
